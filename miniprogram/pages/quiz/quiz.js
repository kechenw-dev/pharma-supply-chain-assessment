...
Page({
  data: { questions: [], options: OPTIONS, index: 0, current: null, selected: '', progress: '', progressPercent: 0, dimension: '' },
  onLoad() {
  ...
  showQuestion(index) {
    ...
    this.setData({ index, current: q, selected: draft.answers[q.id] || '', progress: `${index + 1} / ${this.data.questions.length}`, progressPercent: Math.round((index + 1) / this.data.questions.length * 100), dimension: dim ? dim.title : '与你的岗位相关' })
  },
